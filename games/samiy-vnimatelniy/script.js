function initSamiyVnimatelniy(){
  ...
  checkBtn.addEventListener('click',()=>{ const got=slots.map(s=> s.firstChild && s.firstChild.dataset.word); const ok = JSON.stringify(got)===JSON.stringify(cur); feedback.textContent= ok?'Отлично!':'Порядок другой'; feedback.style.color= ok? '#065f46':'#7f1d1d'; if(ok){ setTimeout(()=>{ cur=sets[Math.floor(Math.random()*sets.length)]; render(); }, 1000); } });
  render();
}
